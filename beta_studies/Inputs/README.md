...
```
Where the values inside ```example.in``` give the folder name. Must insure than the inputs in the folder name are listed in the input file. This will also allow you to track the changes that are being made to the output file. Do this by moving into the newly created folder then monitor the progress:
```bash
cd new folder
tail -f run.log
```
