...
def plot_real_imag_components(nc_filepaths, labels=None, target_ky=None): 
    ...
    plt.tight_layout()
    plt.savefig('Figures/CarbonZeff_beta4.0.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        try:
            ...
            # Ask the user for labels
        labels = []

        print("\nEnter labels for each file:\n")

        for fp in file_paths:
            label = input(f"Label for {fp}: ")
            
            # fallback if user presses Enter
            if label.strip() == "":
                label = fp
